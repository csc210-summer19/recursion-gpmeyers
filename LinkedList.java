 ...
 * 
 * @author Rick Mercer and Graysen Meyers
 */
public class LinkedList<E extends Comparable<E>> {
	...
	public void removeAll(E el) {
		...
		removeAll(el, first, 0, size() - 1);
	}

	private void removeAll(E el, Node ref, int startIndex, int stopIndex) {
		if (startIndex != stopIndex) {
			Node temp;
			if (first == ref && ref.data.equals(el)) {
				first = first.next;
				temp = first;
				n--;
			...
			removeAll(el, temp, startIndex + 1, stopIndex);
		}
	}
